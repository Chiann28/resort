...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($room_id, $room_ids_with_limit) && $total_guests > $guests_limit) {
        echo "<script>alert('Error: The total number of adults and children cannot exceed $guests_limit for the selected room.'); window.location.href = 'user_dashboard.php';</script>";
        exit();
    }

    if (in_array($room_id, $room_ids_with_individual_limit) && $total_guests > $individual_limit) {
        echo "<script>alert('Error: Room $room_id has a maximum limit of $individual_limit guests.'); window.location.href = 'user_dashboard.php';</script>";
        exit();
    }

    if ($room_id == 15 && $total_guests > 2) {
        echo "<script>alert('Error: The total number of adults and children cannot exceed 2 for this room.'); window.location.href = 'user_dashboard.php';</script>";
        // You can redirect the user back to the reservation page or take other actions as needed.
        exit();
    }

    if ($room_id == 20 && $total_guests > 12) {
        echo "<script>alert('Error: The total number of adults and children cannot exceed 12 for this room.'); window.location.href = 'user_dashboard.php';</script>";
        // You can redirect the user back to the reservation page or take other actions as needed.
        exit();
    }

    if ($room_id == 21 && $total_guests > 18) {
        echo "<script>alert('Error: The total number of adults and children cannot exceed 18 for this room.'); window.location.href = 'user_dashboard.php';</script>";
        // You can redirect the user back to the reservation page or take other actions as needed.
        exit();
    }
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        echo "<script>alert('$error_message'); window.location.href = 'user_dashboard.php';</script>";
        exit();
    }
}
?>
